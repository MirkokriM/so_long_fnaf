...
void	direction_w(int id, t_game *game)
{
	if (id == W || id == UP)
	{
		if (game->vars->map[game->map_i - 1][game->map_j] != '1')
		{
			...
			else if (game->vars->map[game->map_i - 1][game->map_j] \
				== 'E' && game->vars->index_collect <= 0)
				exit_game(game, 1);
			else if (game->vars->map[game->map_i - 1][game->map_j] \
				== 'E' && game->vars->index_collect > 0)
				return ;
	...
	if (id == A || id == LEFT)
	{
		if (game->vars->map[game->map_i][game->map_j - 1] != '1')
		{
			...
			if (game->vars->map[game->map_i][game->map_j - 1] \
				== 'E' && game->vars->index_collect <= 0)
				exit_game(game, 1);
			else if (game->vars->map[game->map_i][game->map_j - 1] \
				== 'E' && game->vars->index_collect > 0)
				return ;
			...
			mlx_put_image_to_window(game->graph.mlx, game->graph.win, \
				game->graph.img_terrain, game->x_arrow, game->y_arrow);
			mlx_put_image_to_window(game->graph.mlx, game->graph.win, \
				game->graph.img_player, \
					game->x_arrow - TILESIZE, game->y_arrow);
		}
	}
}
...
void	direction_d(int id, t_game *game)
{
	if (id == D || id == RIGHT)
	{
		if (game->vars->map[game->map_i][game->map_j + 1] != '1')
		{
			if (game->vars->map[game->map_i][game->map_j + 1] == 'C')
				game->vars->index_collect--;
			if (game->vars->map[game->map_i][game->map_j] \
				== 'E' && game->vars->index_collect <= 0)
				exit_game(game, 1);
			else if (game->vars->map[game->map_i][game->map_j +1] \
				== 'E' && game->vars->index_collect > 0)
